...
const RE_CONTROL_META_ERROR =
  /^\/devices\/([^/]+)\/controls\/([^/]+)\/meta\/error$/;
const RE_CONTROL_META_SUBTOPIC =
...
export class WirenboardMqtt extends EventEmitter {
  ...
  async start(): Promise<void> {
    ...
    const port =
      mqttPort ??
      (mqttProtocol === "mqtts" || mqttProtocol === "wss" ? 8883 : 1883);
    ...
    if (mqttProtocol === "mqtts" || mqttProtocol === "wss") {
      if (this.config.mqttCaPath)
        options.ca = fs.readFileSync(this.config.mqttCaPath);
      if (this.config.mqttCertPath)
        options.cert = fs.readFileSync(this.config.mqttCertPath);
      if (this.config.mqttKeyPath)
        options.key = fs.readFileSync(this.config.mqttKeyPath);
    }
    ...
    if (m) {
      ...
      if (payload.startsWith("{")) {
        try {
          ...
          this.log.warn(
            `Failed to parse control meta JSON for ${deviceName}/${controlName}`,
          );
        }
      }
      return;
