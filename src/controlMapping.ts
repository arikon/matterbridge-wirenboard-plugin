...
import { WbControlMeta, WbControlType, WbDeprecatedControlType } from './wirenboardTypes.js';
...
const DEPRECATED_TYPE_MAP: Partial<Record<WbDeprecatedControlType, { type: 'value'; units: string }>> = {
...
export const CONTROL_MAPPINGS: WbToMatterMapping[] = [
  ...
  // doorLock (name-based)
  {
    ...
    reverseConverter: (v) =>
      v === DoorLock.LockState.Locked ? '1' : '0',
  },
  // fan (name-based)
  {
    wbType: 'switch',
  ...
  // ── alarm ─────────────────────────────────────────────────────────────────
  {
    ...
    converter: (v) =>
      v === '1' ? SmokeCoAlarm.AlarmState.Critical : SmokeCoAlarm.AlarmState.Normal,
  },
  {
    wbType: 'alarm',
  ...
  // ── range ─────────────────────────────────────────────────────────────────
  {
    ...
    converter: (v, meta) => rangeToLiftPercent100ths(parseFloat(v), meta.max ?? 255),
  ...
  // range fallback → dimmableLight
  {
    ...
    converter: (v, meta) => rangeToLevel(parseFloat(v), meta.min ?? 0, meta.max ?? 255),
    reverseConverter: (v, meta) =>
      String(levelToRange(v as number, meta.min ?? 0, meta.max ?? 255)),
  },
  ...
  // ── dimmer ────────────────────────────────────────────────────────────────
  {
    ...
    converter: (v, meta) => rangeToLevel(parseFloat(v), meta.min ?? 0, meta.max ?? 65535),
    reverseConverter: (v, meta) =>
      String(levelToRange(v as number, meta.min ?? 0, meta.max ?? 65535)),
  },
  ...
  // Temperature
  {
    ...
    reverseConverter: (v, meta) => String(roundToPrecision((v as number) / 100, meta.precision)),
  },
  // Humidity
  {
    wbType: 'value',
  ...
  // Pressure
  {
    ...
    reverseConverter: (v, meta) => String(roundToPrecision((v as number) / 0.01, meta.precision)),
  },
  {
    wbType: 'value',
    wbUnits: 'mbar',
    ...
    reverseConverter: (v, meta) => String(roundToPrecision(v as number, meta.precision)),
  },
  {
    wbType: 'value',
    wbUnits: 'bar',
    ...
    reverseConverter: (v, meta) => String(roundToPrecision((v as number) / 1000, meta.precision)),
  },
  // Illuminance
  {
    wbType: 'value',
  ...
  },
  {
    ...
    converter: (v) => Math.round(parseFloat(v) * 1_000_000),
    reverseConverter: (v) => String((v as number) / 1_000_000),
  },
...
export function findMapping(
  ...
  if (deviceOverrides && controlName && deviceOverrides[controlName]) {
    ...
    for (const mapping of CONTROL_MAPPINGS) {
      ...
      if (mapping.wbUnits !== undefined && mapping.wbUnits !== normalizedMeta.units) continue;
  ...
  if (nameLower) {
    for (const mapping of CONTROL_MAPPINGS) {
      ...
      if (mapping.wbType !== normalizedMeta.type) continue;
      if (mapping.wbUnits !== undefined && mapping.wbUnits !== normalizedMeta.units) continue;
      if (mapping.readonly !== undefined && mapping.readonly !== (normalizedMeta.readonly ?? false)) continue;
