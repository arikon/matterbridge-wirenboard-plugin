...
Resolution SHALL use **Matterbridge export names** (e.g. `onOffOutlet`) matching `DeviceTypeDefinition` values that appear in `CONTROL_MAPPINGS`, not Matter device type name strings such as `MA-onoffpluginunit`. The JSON Schema `enum` for `deviceOverrides.controls.*.deviceType` SHALL be produced at plugin build from that set. Types used only by composite assembly (e.g. thermostat built from multiple controls) SHALL NOT appear in the enum unless they appear as `matterDeviceType` in `CONTROL_MAPPINGS`.
...
The source layout SHALL include a template (`matterbridge-wirenboard-plugin.schema.template.json`) whose static fields are edited by hand; the `deviceOverrides` Matter `deviceType` **enum** SHALL be generated into the published schema during the plugin build from `MATTER_DEVICE_TYPE_BY_NAME` (see device overrides configuration).
