...
import type { DeviceMetaEvent, ControlMetaEvent, ControlValueEvent, ControlErrorEvent } from '../../src/wirenboardMqtt.js';
...
async function startBroker(): Promise<{ broker: Aedes; server: Server; port: number }> {
...
function aedesPublish(broker: Aedes, topic: string, payload: string, retain = true): Promise<void> {
  return new Promise((resolve) => {
...
async function startAndWaitConnected(
  ...
  const connected = new Promise<void>((resolve, reject) => {
    const timer = setTimeout(() => reject(new Error('Timeout waiting for clientReady')), timeoutMs);
...
function waitForEvents<T>(
  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      reject(new Error(`Timeout waiting for ${count} '${event}' events, got ${collected.length}`));
    }, timeoutMs);
...
function waitForEvent<T>(emitter: WirenboardMqtt, event: string, timeoutMs = 8000): Promise<T> {
...
describe('a. Retained messages discovery', () => {
  ...
  it('receives retained device-meta, control-meta, control-value after connect', async () => {
    ...
    await aedesPublish(broker, '/devices/wb-mr6c/meta', JSON.stringify({ driver: 'wb-modbus', title: 'WB-MR6C' }));
    await aedesPublish(broker, '/devices/wb-mr6c/controls/K1/meta', JSON.stringify({ type: 'switch', order: 1, readonly: false }));
    ...
    const controlMetas = waitForEvents<ControlMetaEvent>(mqtt, 'control-meta', 1);
    const controlValues = waitForEvents<ControlValueEvent>(mqtt, 'control-value', 1);

    await mqtt.start();

    const [dm, cm, cv] = await Promise.all([deviceMetas, controlMetas, controlValues]);
  ...
  it('publishes to /devices/<dev>/controls/<ctrl>/on', async () => {
    ...
    const { broker: broker2, server: server2 } = await (async () => {
      const mqttConnectPromise = new Promise<void>((resolve, reject) => {
        const timer = setTimeout(() => reject(new Error('Timeout waiting for mqtt_connect')), 12000);
        mqtt.once('mqtt_connect', () => { clearTimeout(timer); resolve(); });
      });
    ...
    const messageReceived = new Promise<void>((resolve, reject) => {
      const timer = setTimeout(() => reject(new Error('Timeout waiting for /on message')), 5000);
      spyClient.on('message', () => {
        if (received.some((m) => m.topic === '/devices/wb-mr6c/controls/K1/on')) {
          clearTimeout(timer);
          resolve();
        }
      });
    ...
    const msg = received.find((m) => m.topic === '/devices/wb-mr6c/controls/K1/on')!;
...
describe('d. Reconnect', () => {
  it('reconnects after broker restart', async () => {
    const { broker: broker1, server: server1, port: localPort } = await startBroker();
    ...
    await new Promise<void>((resolve) => server2.listen(localPort, () => resolve()));
...
describe('e. Empty payload (device removal)', () => {
  ...
  it('emits device-removed on empty meta payload', async () => {
    ...
    const removePromise = waitForEvent<{ deviceName: string }>(mqtt, 'device-removed');
...
describe('f. Error topics', () => {
  ...
  it('emits device-error from /devices/<dev>/meta/error', async () => {
    ...
    const errPromise = waitForEvent<{ deviceName: string; error: string }>(mqtt, 'device-error');
  ...
  it('emits control-error from /devices/<dev>/controls/<ctrl>/meta/error', async () => {
    ...
    await aedesPublish(broker, '/devices/wb-err/controls/K1/meta/error', 'w', false);
...
describe('h. Full WB device simulation', () => {
  ...
  it('discovers device with multiple controls from retained messages', async () => {
    await aedesPublish(broker, '/devices/wb-full/meta', JSON.stringify({ driver: 'wb-modbus', title: 'Full Device' }));
    await aedesPublish(broker, '/devices/wb-full/controls/Channel1/meta', JSON.stringify({ type: 'switch', order: 1, readonly: false }));
    await aedesPublish(broker, '/devices/wb-full/controls/Channel2/meta', JSON.stringify({ type: 'switch', order: 2, readonly: false }));
    await aedesPublish(broker, '/devices/wb-full/controls/Temperature/meta', JSON.stringify({ type: 'value', order: 3, readonly: true, units: 'deg C' }));
    ...
    const mqtt = createWbMqtt(port);
    const deviceMetas = waitForEvents<DeviceMetaEvent>(mqtt, 'device-meta', 1);
    const controlMetas = waitForEvents<ControlMetaEvent>(mqtt, 'control-meta', 3);
    const controlValues = waitForEvents<ControlValueEvent>(mqtt, 'control-value', 3);
